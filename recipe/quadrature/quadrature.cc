...
double QuadratureDoubleExponentialFinite(const Integral& integral,
                                         const int num_partition,
                                         const double length) {
  return QuadratureDoubleExponentialFinite(integral.GetIntegrand(),
                                           num_partition, length);
}
...
double QuadratureDoubleExponentialHalf(const Integral& integral,
                                       const int num_partition,
                                       const double length) {
  return QuadratureDoubleExponentialHalf(integral.GetIntegrand(), num_partition,
                                         length);
}
...
double QuadratureDoubleExponentialInfinite(const Integrand& integrand,
  ...
  for (int i = -num_partition + 1; i < num_partition; i++) {
    const double factor = RECIPE_PI * 0.5 * std::sinh(i * length);
    sum += integrand(std::sinh(factor)) * std::cosh(i * length) *
           std::cosh(factor);
  }
  sum *= RECIPE_PI * 0.5 * length;
  return sum;
...
double QuadratureDoubleExponentialInfinite(const Integral& integral,
                                           const int num_partition,
                                           const double length) {
  return QuadratureDoubleExponentialInfinite(integral.GetIntegrand(),
                                             num_partition, length);
}
...
double TrapezoidalRuleOpenDegree5(const Integrand& integrand, const double left,
  ...
  // calculate endpoints
  double integral =
      (integrand(left + h) * 55.0 / 24.0 - integrand(left + 2.0 * h) / 6.0 +
